...
import { RLAnalyticsChart } from "@/components/dashboard/RLAnalyticsChart";
import { LearningStatusBar } from "@/components/dashboard/LearningStatusBar";
...
export default function Dashboard() {
  ...
  const { metrics, taxis, zones, gridSize, history, rlAnalytics } = state;
      ...
      <div className="max-w-[1600px] mx-auto space-y-4">
        ...
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-4 border-b border-border">
          <div>
            <div className="flex items-center gap-3 mb-1 relative">
              <div className="absolute -inset-1 bg-gradient-to-r from-primary/20 to-accent/20 blur-xl rounded-full opacity-50 animate-pulse-slow" />
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {isRunning ? <><Square className="w-4 h-4 fill-current" /> Pause</> : <><Play className="w-4 h-4 fill-current" /> Start</>}
            </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-6 xl:col-span-7 flex flex-col gap-4">
            ...
            <div className="bg-card border border-card-border rounded-xl p-5 shadow-sm flex flex-col min-h-[300px]">
              <div className="mb-3">
                <h3 className="font-semibold text-lg tracking-tight">RL Agent Analytics</h3>
                <p className="text-xs text-muted-foreground">Reward history · Q-convergence · ε decay</p>
              </div>
              <div className="flex-1 min-h-0">
                <RLAnalyticsChart data={history} analytics={rlAnalytics} />
              </div>
            </div>
          </div>
        </div>
        ...
        {/* ── Taxi Fleet Table ── */}
        <div className="mt-2">
          <TaxiTable taxis={taxis} debugMode={debugMode} />
        </div>
